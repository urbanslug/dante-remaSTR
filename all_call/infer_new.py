...
class Inference:
    ...
    def infer(self, annotations: list[Annotation], filt_annotations: list[Annotation], index_rep: int,
        ...
        observed_annots = list(map(lambda ann: ann.module_repetitions[index_rep], annotations))
        rl_annots = list(map(lambda ann: len(ann.read_seq), annotations))
        closed_annots = np.ones_like(observed_annots, dtype=bool)

        # generate open observed and read_length arrays
        observed_fa = list(map(lambda ann: ann.module_repetitions[index_rep], filt_annotations))
        rl_fa = list(map(lambda ann: len(ann.read_seq), filt_annotations))
